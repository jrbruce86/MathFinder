...
mainWindow::mainWindow() : highlightdone(false), inZoomMode(false), inSelectMode(false), selecting(false), zoomcount(0), scaleFactor(1.0)
{
...
void mainWindow::tab_changed()
{
	...
	if(tabLayout->currentIndex() > -1) {
		...
		{
			rect = QRect(QPoint(0, 0), QPoint(currentTab->scanImage->width(), currentTab->scanImage->height()));
			currentTab->image = new QImage((QString)(currentTab->getImagePath().c_str()));
...
void mainWindow::file_new()
{
	...
	tempTab = new pageTab( (int)tabList.size(), ("New"+(int)tabList.size()), ("New"+(int)tabList.size()) );
...
void mainWindow::file_open() {
	...
	pageTab *tempTab;
	list<pageTab*>::iterator iter = tabList.begin();
	path  = QFileDialog::getOpenFileName(this, "Open file", "Images or Text (*.png *.pnm *.tif *.txt *.html *.xml *.gif)");
	if (path.contains(".tif", Qt::CaseInsensitive) || path.contains(".png", Qt::CaseInsensitive) 
	|| path.contains(".pnm", Qt::CaseInsensitive) || path.contains(".gif", Qt::CaseInsensitive)) {
		path.replace(path.lastIndexOf((QString)(".")), path.size() 
			- path.lastIndexOf((QString)(".")), ".png"); //png should be the output name
		QFile file(path);
		if(file.size() < 10500179) { // only try to open it if its less than 10 MB
			...
			QMessageBox errorBox( tr("File Open Error"), tr("File Open: Files cannot be larger than 10 MB. Try a smaller file, or \
try compressing this one.\n"), QMessageBox::Critical, QMessageBox::Ok, QMessageBox::NoButton, QMessageBox::NoButton, this );
			errorBox.exec();
		}
	}
	else if (path.contains(".txt", Qt::CaseInsensitive) || path.contains(".html", Qt::CaseInsensitive)) {
		tempTab = new pageTab( (int)tabList.size(), (string)path.toStdString(), "" );
		tabList.push_back(tempTab);
...
void mainWindow::writecoordsdisp() {
	...
	scaledRect = QRect(QPoint(rect.topLeft() / resizefactor), QPoint(rect.bottomRight() / resizefactor));
	...
	gtWrite << "Image" << currentTab->getImageFilename().c_str() << 
	": displayed expression :" << "(" << tl_x << "," << tl_y << ")(tl)" <<
	"(" << br_x << "," << br_y << ")(br)\n";
	gtWrite.flush();
}
...
void mainWindow::writecoordsemb() {
	...
	int br_y = scaledRect.bottomRight().y();
	gtWrite << "Image" << currentTab->getImageFilename().c_str() << 
	": embedded expression :" << "(" << tl_x << "," << tl_y << ")(tl)" <<
	"(" << br_x << "," << br_y << ")(br)\n";
	gtWrite.flush();
}
...
{
	list<pageTab*>::iterator iter = tabList.begin();
	while ( ((*iter)->getIndex() != tabLayout->currentIndex()) && (iter != tabList.end()) )
		iter++;
		
	(*iter)->viewImage();
	tabLayout->setTabText( tabLayout->currentIndex(), tr(((*iter)->getImageFilename()).c_str()) );
}
...
void mainWindow::createActions()
{
	...
	action_file_import_img->setToolTip( tr("Import an image into the current tab.") );
	connect( action_file_import_img, SIGNAL(triggered()), this, SLOT(import_image()) );
	...
	//File Toolbar
	action_fileT_open = new QAction( QIcon(":/icons/open_icon.png"), tr("Open"), this );
	action_fileT_open->setToolTip( action_file_open->toolTip() );
	connect( action_fileT_open, SIGNAL(triggered()), this, SLOT(file_open()) );

	action_writecoordsdisp = new QAction(QIcon(":/icons/ptmd.png"), tr("write disp coordinates"), this);
	connect(action_writecoordsdisp, SIGNAL(triggered()), this, SLOT(writecoordsdisp())); 

	action_writecoordsemb = new QAction(QIcon(":/icons/ptm.png"), tr("write emb coordinates"), this);
	connect(action_writecoordsemb, SIGNAL(triggered()), this, SLOT(writecoordsemb())); 
	...
	connect( action_view_tool_file, SIGNAL(triggered()), this, SLOT(view_tool_file()) );
...
void mainWindow::scaleImage(double factor) {
	scaleFactor *= factor;
	currentTab->imgView->resize(scaleFactor * currentTab->imgView->pixmap()->size());
...
MouseEventEater::MouseEventEater(mainWindow* parent) : myparent(parent) {
}

bool MouseEventEater::eventFilter(QObject* obj, QEvent* event) {
	if(((event->type() == QEvent::MouseButtonPress) || (event->type() == QEvent::Wheel)) && (myparent->inZoomMode)) {
	...
	else if((event->type() == QEvent::MouseButtonPress) && (myparent->inSelectMode)) {
		...
		QPixmap original;
		myparent->currentTab->imgView->setPixmap(*(myparent->currentTab->scanImage));
		myparent->selecting = true;
		return true;
	}
	else if((event->type() == QEvent::MouseMove) && (myparent->inSelectMode) && (myparent->selecting)) {
		QMouseEvent *mouseevent = static_cast<QMouseEvent*>(event);
		QPoint pos_calc = mouseevent->pos();
		pos_calc.rx() /= pow((double)1.25, myparent->zoomcount);
		pos_calc.ry() /= pow((double)1.25, myparent->zoomcount);
