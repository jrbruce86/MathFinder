 ...
 * File name:   F_Ext3.cpp
 * Written by:  Jake Bruce, Copyright (C) 2014
 * History:     Created Jan 1, 2014 1:28:46 PM
 * ------------------------------------------------------------------------
 ...
 *              feature combination, i.e., without the the valid_row and 3 N-Gram
 *              features.
 * ------------------------------------------------------------------------
...
void F_Ext3::initFeatExtSinglePage() {
 ...
 // dbgim = pixConvertTo32(dbgim);
  ...
  grid->setFeatExtFormat(training_set_path, "F_Ext3", (int)NUM_FEATURES);
...
std::vector<double> F_Ext3::extractFeatures(tesseract::BLOBINFO* blob) {
